          ...
          foreach ($workTime->getShifts() as $wtShift) {
            ...
            foreach (($params['busyShifts'] ?? []) as $bsShift) {
              ...
              if ($startDate->toDateString() == $bsShift['calendarDate']) {
                ...
                if ($shiftStartTime->between($bsStartTime, $bsEndTime) && $shiftStartTime->lessThan($bsEndTime)) $isBusy = 1;
                if ($shiftEndTime->between($bsStartTime, $bsEndTime) && $shiftEndTime->greaterThan($bsStartTime)) $isBusy = 1;
                if ($bsStartTime->between($shiftStartTime, $shiftEndTime) && $bsStartTime->lessThan($shiftEndTime)) $isBusy = 1;
                if ($bsEndTime->between($shiftStartTime, $shiftEndTime) && $bsEndTime->greaterThan($shiftStartTime)) $isBusy = 1;
